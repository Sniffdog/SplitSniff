$Path = '.\Bin\NVIDIA-KlausT6\ccminer.exe'
...
$Optimizations = [PSCustomObject]@{
    ...
    X11 = ''
    MyriadGroestl = ''
    Groestl = ' -d $SplitSniffCC6'
    Keccak = ''
    Scrypt = ''
    Nist5 = ' -d $SplitSniffCC6'
}
...
$Algorithms | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | ForEach {
    [PSCustomObject]@{
        Type = 'NVIDIA6'
        Path = $Path
        Arguments = -Join ('-a ', $Algorithms.$_, ' -o stratum+tcp://$($Pools.', $_, '.Host):$($Pools.', $_, '.Port) -b 0.0.0.0:4074 -u $($Pools.', $_, '.User6) -p $($Pools.', $_, '.Pass)', $Optimizations.$_)
